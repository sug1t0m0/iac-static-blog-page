resource "aws_cloudfront_distribution" "images_cdn" {
  ...
  origin {
    domain_name = aws_s3_bucket.images_storage.bucket_regional_domain_name
    origin_id   = aws_s3_bucket.images_storage.id
    s3_origin_config {
  ...
  default_cache_behavior {
    ...
    target_origin_id = aws_s3_bucket.images_storage.id
